...
exports.Project = exports.PeerWarningType = exports.InstallMode = exports.LOCKFILE_VERSION = void 0;
...
exports.LOCKFILE_VERSION = miscUtils.parseInt(process.env.YARN_LOCKFILE_VERSION_OVERRIDE ??
    8);
...
var PeerWarningType;
(function (PeerWarningType) {
    PeerWarningType[PeerWarningType["NotProvided"] = 0] = "NotProvided";
    PeerWarningType[PeerWarningType["NotCompatible"] = 1] = "NotCompatible";
    PeerWarningType[PeerWarningType["NodeNotProvided"] = 2] = "NodeNotProvided";
    PeerWarningType[PeerWarningType["NodeNotCompatible"] = 3] = "NodeNotCompatible";
})(PeerWarningType || (exports.PeerWarningType = PeerWarningType = {}));
...
class Project {
    static async find(configuration, startingCwd) {
        ...
        Configuration_1.Configuration.telemetry?.reportDependencyCount(project.workspaces.reduce((sum, workspace) => sum + workspace.manifest.dependencies.size + workspace.manifest.devDependencies.size, 0));
    ...
    async setupResolutions() {
        ...
        if (fslib_2.xfs.existsSync(lockfilePath)) {
            ...
            if (parsed.__metadata) {
                ...
                this.lockfileNeedsRefresh = lockfileVersion < exports.LOCKFILE_VERSION;
    ...
    async setupWorkspaces() {
        ...
        const limitSetup = (0, p_limit_1.default)(4);
    ...
    addWorkspace(workspace) {
        ...
        if (typeof dup !== `undefined`)
            throw new Error(`Duplicate workspace name ${structUtils.prettyIdent(this.configuration, workspace.anchoredLocator)}: ${fslib_1.npath.fromPortablePath(workspace.cwd)} conflicts with ${fslib_1.npath.fromPortablePath(dup.cwd)}`);
    ...
    async resolveEverything(opts) {
        ...
        const peerWarnings = [];
        const peerRequirementNodes = new Map();
    ...
    async linkEverything({ cache, report, fetcher: optFetcher, mode }) {
        ...
        const nextSkippedBuilds = new Set();
                    ...
                    for (const directive of buildInfo.buildDirectives) {
                        ...
                        const wasBuildSuccessful = await fslib_2.xfs.mktempPromise(async (logDir) => {
                            ...
                            if (StreamReport_1.SUPPORTS_GROUPS)
                                report.reportFold(fslib_1.npath.fromPortablePath(logFile), fslib_2.xfs.readFileSync(logFile, `utf8`));
                            return isOptional;
                        });
    ...
    generateLockfile() {
        ...
        optimizedLockfile.__metadata = {
            version: exports.LOCKFILE_VERSION,
            cacheKey,
        };
 ...
 */
function applyVirtualResolutionMutations({ project, allDescriptors, allResolutions, allPackages, accessibleLocators = new Set(), optionalBuilds = new Set(), peerRequirements = new Map(), peerWarnings = [], peerRequirementNodes = new Map(), volatileDescriptors = new Set(), }) {
    ...
    const resolvePeerDependenciesImpl = (parentDescriptor, parentLocator, parentPeerRequests, { top, optional }) => {
        ...
        const parentPeerRequirements = new Map();
        const firstPass = [];
        const secondPass = [];
        const thirdPass = [];
        const fourthPass = [];
            ...
            if (!satisfiesAll) {
                ...
                peerWarnings.push({
                    ...
                    range: allRanges.includes(null)
                        ? null
                        : semverUtils.simplifyRanges(allRanges),
